...
def transform_data(endpoint_url, access_key, secret_key):
    ...
    # Khởi tạo s3fs để Pandas ghi trực tiếp lên MinIO
    s3_fs = s3fs.S3FileSystem(
        anon=False,
        key=access_key,
        secret=secret_key,
        client_kwargs={'endpoint_url': f"http://{endpoint_url}"}
    )

    # Khởi tạo MinIOClient để quản lý bucket 
    client_wrapper = MinIOClient(endpoint_url, access_key, secret_key)
    client_wrapper.create_bucket(datalake_cfg['bucket_name_2'])
    ...
    for year in YEARS:
        ...
        for file in all_fps:
            ...
            # Lưu vào bucket 'processed' kèm theo phân cấp năm (year)
            output_path = f"s3://{datalake_cfg['bucket_name_2']}/{datalake_cfg['folder_name']}/{year}/{file_name}"
